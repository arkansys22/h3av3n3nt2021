<?php
    echo "<div class='col-md-12'>
              <div class='box box-info'>
                <div class='box-header'>
                  <h3 class='box-title'>Tambah Video Baru ".$this->session->disabled." </h3>
                </div>
              <div class='box-body'>";
              $attributes = array('class'=>'form-horizontal','role'=>'form');
              echo form_open_multipart('administrator/tambah_listvideo',$attributes);
          echo "<div class='col-md-12'>
                  <table class='table table-condensed'>
                  <tbody>
                    <input type='hidden' name='id' value=''>
                    <tr><th width='120px' scope='row'>Judul</th>    <td><input type='text' class='form-control' name='b' required></td></tr>
                    <td><input type='hidden' class='form-control' name='a' value='2' required></td>
                    <tr><th scope='row'>Gambar 1</th>
                    <td><input type='file' class='form-control' name='img1'></td>
                    </tr>
                    <tr><th scope='row'>LINK YOUTUBE</th>
                        <td><input type='text' class='form-control' name='urlyoutube' required></td>
                    </tr>
                  </tbody>
                  </table>
                </div>

              <div class='box-footer'>
                    <button type='submit' name='submit' class='btn btn-info'>Tambahkan</button>

                  </div>
            </div></div></div>";
            echo form_close();

			?>
